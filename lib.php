...
function choicegroup_get_response_data($choicegroup, $cm) {
    ...
    $users = get_enrolled_users($context, 'mod/choicegroup:choose', 0, user_picture::fields('u', array('idnumber')), 'u.lastname ASC,u.firstname ASC');
    if ($users) {
        $users = \core_availability\info::filter_user_list($users);
    }
